...
def train_model(model, train_loader, valid_loader, criterion, seq_criterion, optimizer, scheduler, save_dir, valid_trues, num_epochs=20):
    ...
    def valid(model, valid_loader, criterion, seq_criterion):
        ...
        for (idx, batch) in enumerate(valid_loader):
            ...
            lengths = lengths.to(device)
            optimizer.zero_grad()
            char_outputs, seq_outputs, seq_length, hidden, char_alpha, word_alpha = model(inputs, lengths, seq_mfcc, sen_mfcc) #mask,
...
def collate_fn(batch):
    ...
    seq_mfcc = torch.stack([torch.FloatTensor(item['otherMat'][0]) for item in batch],dim=0)
    sen_mfcc = torch.stack([torch.FloatTensor(item['otherMat'][1]) for item in batch],dim=0)
    seqlabel = torch.LongTensor([item['seqLabelMat'] for item in batch])
    length = torch.FloatTensor([item['lenMat'] for item in batch])
    return ids, ords, data, label, seq_mfcc, sen_mfcc, seqlabel, length
...
if __name__ == '__main__':
    ...
    proc_ecg_doc = np.load(data_dir +  "proc_ecg_doc.npy", allow_pickle=True).item()
    ...
    for num in seq_index_train:
        ...
        if train_ecg_sen[num][7] == 1:
            ...
            train_oth_Mat[num] = [train_ecg_sen[num][5], train_ecg_doc[num.split('_')[0]][2]]
    ...
    for num in seq_index_valid:
        if valid_ecg_sen[num][7] == 1:
            ...
            valid_oth_Mat[num] = [valid_ecg_sen[num][5], valid_ecg_doc[num.split('_')[0]][2]]
    ...
    for num in seq_index_test:
        if test_ecg_sen[num][7] == 1:
            ...
            test_oth_Mat[num] = [test_ecg_sen[num][5], test_ecg_doc[num.split('_')[0]][2]]
    ...
    train_dataset = data.MMECG(dataMat=train_data_Mat, labelMat=train_label_Mat, LenMat=train_length_Mat,
                                     otherMat=train_oth_Mat, SeqLabelMat=train_seq_lable_Mat, OrdMat=train_ord_Mat,
    ...
    valid_dataset = data.MMECG(dataMat=valid_data_Mat, labelMat=valid_label_Mat, LenMat=valid_length_Mat,
                                     otherMat=valid_oth_Mat, SeqLabelMat=valid_seq_lable_Mat, OrdMat=valid_ord_Mat,
    ...
    test_dataset = data.MMECG(dataMat=test_data_Mat, labelMat=test_label_Mat, LenMat=test_length_Mat,
                                   otherMat=test_oth_Mat, SeqLabelMat=test_seq_lable_Mat, OrdMat=test_ord_Mat,
